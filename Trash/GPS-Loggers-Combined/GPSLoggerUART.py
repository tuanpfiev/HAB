...
def GPSLoggerSocketUblox():

    # set up socket
    try:
        socket_logger = socket.socket(socket.AF_INET, socket.SOCK_STREAM)    
        socket_logger.connect((GlobalVals.HOST, GlobalVals.GPS_UBLOX_LOGGER_SOCKET))
        socket_logger.settimeout(GlobalVals.GPS_UBLOX_LOGGER_SOCKET_TIMEOUT)
    except Exception as e:
        print("Exception: " + str(e.__class__))
        print("There was an error starting the logger socket. This thread will now stop.")
        with GlobalVals.BREAK_GPS_LOGGER_UBLOX_THREAD_MUTEX:
            GlobalVals.BREAK_GPS_LOGGER_UBLOX_THREAD = True
        return 
    ...
    while True:
        ...
        with GlobalVals.BREAK_GPS_LOGGER_UBLOX_THREAD_MUTEX:
            if GlobalVals.BREAK_GPS_LOGGER_UBLOX_THREAD:
                break
        ...
        if synced and bufferRead == 32:
            ...
            GPSData =GPS(None,Latitude,Longitude,Altitude,GPSTime)

            # add data to the gps buffer 
            with GlobalVals.GPS_UBLOX_DATA_BUFFER_MUTEX:
                GlobalVals.GPS_UBLOX_DATA_BUFFER.append(GPSData)
...
def GPSLoggerSocketQuectel():

    # set up socket
    try:
        socket_logger = socket.socket(socket.AF_INET, socket.SOCK_STREAM)    
        socket_logger.connect((GlobalVals.HOST, GlobalVals.GPS_QUECTEL_LOGGER_SOCKET))
        socket_logger.settimeout(GlobalVals.GPS_QUECTEL_LOGGER_SOCKET_TIMEOUT)
    except Exception as e:
        print("Exception: " + str(e.__class__))
        print("There was an error starting the logger socket. This thread will now stop.")
        with GlobalVals.BREAK_GPS_LOGGER_QUECTEL_THREAD_MUTEX:
            GlobalVals.BREAK_GPS_LOGGER_QUECTEL_THREAD = True
        return 
    ...
    while True:
        ...
        # if flag is set break the thread 
        with GlobalVals.BREAK_GPS_LOGGER_QUECTEL_THREAD_MUTEX:
            if GlobalVals.BREAK_GPS_LOGGER_QUECTEL_THREAD:
                break
        ...
        if synced and bufferRead == 32:
            ...
            # add data to the gps buffer 
            with GlobalVals.GPS_QUECTEL_DATA_BUFFER_MUTEX:
                GlobalVals.GPS_QUECTEL_DATA_BUFFER.append(GPSData)
