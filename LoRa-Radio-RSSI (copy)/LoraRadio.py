...
def main(StartState):
    ...
    while connected:
        ...
        if GlobalVals.TIMECHECK == 1:
            GlobalVals.TIMESEND = [0,2]
        elif GlobalVals.TIMECHECK == 2:
            GlobalVals.TIMESEND = [4,6]

        # if inRangeCheck(math.floor(timeCheck)%8,GlobalVals.TIMESEND):
        # if math.floor(timeCheck) % 3 == GlobalVals.TIMECHECK:
